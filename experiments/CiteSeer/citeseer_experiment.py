...
class GraphBioNetwork(nn.Module):
    def __init__(self, num_node_features, num_classes=7,enable_monitoring=False, disable_monitoring=False):
        ...
        self.bio_layers = nn.ModuleList([
            BioLogicalNeuron(hidden_dim * 6, 1024, repair_threshold=0.95, repair_intensity=0.015, plasticity_rate=0.0015,summary_interval=5,enable_monitoring=monitoring_state,log_file='bio_layer1.log'),
            BioLogicalNeuron(1024, 512, repair_threshold=0.95, repair_intensity=0.015, plasticity_rate=0.0015,summary_interval=5,enable_monitoring=monitoring_state,log_file='bio_layer2.log'),
            BioLogicalNeuron(512, 256, repair_threshold=0.95, repair_intensity=0.015, plasticity_rate=0.0015,summary_interval=5,enable_monitoring=monitoring_state,log_file='bio_layer3.log'),
        ])
...
class CiteSeerTrainer:
    ...
    def train_and_evaluate(self):
        if self.wandb_logging:
            wandb.init(project="Citeseer_GraphBiological_CV")
        ...
        with open('citeseer_cv_results.json', 'w') as f:
