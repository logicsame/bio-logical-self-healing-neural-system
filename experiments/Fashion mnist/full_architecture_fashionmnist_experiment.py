...
def get_training_components(model, lr=0.002):  
    """Enhanced training components setup"""
...
class FashionMNISTrainer:
    ...
    def _prepare_dataset(self):
        return datasets.FashionMNIST(root='./data', train=True, download=True)
    ...
    def train_and_evaluate(self):
        ...
        for fold, (train_idx, val_idx, test_idx) in enumerate(cv_progress):
            ...
            model = ModernBioNetwork(num_classes=10,enable_monitoring=self.enable_monitoring,disable_monitoring=self.disable_monitoring)
            model.features[0].conv = weight_norm(nn.Conv2d(1, 64, 3, padding=1))
