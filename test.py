...
class GraphBiologicalNetwork(nn.Module):
    def __init__(self, num_node_features, num_classes=2):
        ...
        self.bio_layers = nn.ModuleList([
            BioLogicalNeuron(jk_dim, 256, log_file='neuron_1', summary_interval=50,repair_threshold=0.8),
            BioLogicalNeuron(256, 128, log_file='neuron_2', summary_interval=50,repair_threshold=0.8),
            BioLogicalNeuron(128, 64, log_file='neuron_3', summary_interval=50,repair_threshold=0.8)
        ])
    ...
    def forward(self, data):
        ...
        for bio_layer in self.bio_layers:
            # Pass gradients to the BioLogicalNeuron
            x, health_report = bio_layer(x, gradients=None)  # Pass gradients if available
